...
test.describe('item type flows ', () => {
  test('basic item type flow', async ({ page }) => {
    ...
    await test.step('create', async () => {
      ...
      await expect(playwrightItemTypePage.locators.nameInput).toHaveValue('test item1');
    ...
    await test.step('edit', async () => {
      ...
      await expect(playwrightItemTypePage.locators.nameInput).toHaveValue('test item1 CHANGED');
      await expect(playwrightItemTypePage.locators.divItemTypeUnit.locator('input')).toHaveValue(
        'meter  (m)'
      );
    });
    ...
    await test.step('delete', async () => {
      ...
      const newDivTestItem2 = page.getByTestId('item-types-list').getByText('test item1 CHANGED');
      await expect(newDivTestItem2).toHaveCount(0);
    });
  });

  test('item type flow with price', async ({ page }) => {
    const playwrightDev = new PlaywrightBasePage(page);
    const playwrightItemTypePage = new PlaywrightItemTypePage(page);
    ...
    await test.step('create', async () => {
      ...
      await expect(playwrightItemTypePage.locators.priceInput).toHaveValue('10.123');
    });
    ...
    await test.step('edit', async () => {
      ...
      await expect(playwrightItemTypePage.locators.priceInput).toHaveValue('11.321');
    });
  ...
  test('item type flow with children', async ({ page }) => {
    ...
    await test.step('create', async () => {
      ...
      expect(await (await playwrightItemTypePage.locators.radioProductCost).innerText()).toBe(
        `what you produce for $${1 + 20 + 60}`
      );
    });
    ...
    await test.step('clone root_parent', async () => {
      ...
      expect(await (await playwrightItemTypePage.getChildInput('child1')).inputValue()).toBe('30');
      expect(await (await playwrightItemTypePage.getChildInput('child2')).inputValue()).toBe('90');
      await expect(playwrightItemTypePage.locators.nameInput).toHaveValue(
